## Coursera - Exploratory Data Analysis - Project COurse 1 - Plot1
...
DataAndTime<-strptime(paste(complete_table$Date, complete_table$Time, sep=" "), "%d/%m/%Y %H:%M:%S") 

##Converts the Global Active Power to Numeric
complete_table$Global_active_power<-as.numeric(complete_table$Global_active_power)
...
hist(subset_table$Global_active_power, col="red", xlab="Global Active Power (kW)", ylab= "Frequency", main = "Global Active Power")
#Closes The Device
dev.off()
